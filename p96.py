...
#  difficulties.
repo = open('p96.txt','r').readlines()
difficulties = [repo[line_num].split()[1].lower() for line_num in range(len(repo)) if line_num % 10 == 0]
repo = [[i for i in line.replace('0','-') if i in ([str(j) for j in range(1,10)] + ['-'])] for line in repo]
sudokus = [repo[i*10+1:i*10+10] for i in range(len(repo)/10)]
...
print sum([int(''.join(i.b[0][0:3])) for i in c])
